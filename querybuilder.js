...
QueryBuilder = new Class({
	 ...
	 * @return
	 */
	 initialize: function(className, filters, extras, justthese)
	{
		this.origin = className;
		this.filters = filters || {};
		this.extras = extras || Array();
		justthese = justthese || Array();
		this.wheres = Array();
		this.joins = Array();
		this.fields = Array();
		this.orders = Array();
		...
		if(!(this.origin.dbSetup)) this.origin = new window[className]();
	 ...
	 * @return
	 */
	buildOrderBy: function()	// filter the 'extras' paramter for order by, group by and limit clauses.
	{
		hasorderby = false;
	 ...
	 * @return
	 */
	 mapFields: function(query, object) // map the 'pretty' fieldnames to db table fieldnames.
	{
		return(query);
	},
	 ...
	 * @return
	 */
	 buildQuery: function() // joins all the previous stuff together.
	{
